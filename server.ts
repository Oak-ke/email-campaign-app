...
const forceIPv4CustomLookup = (hostname: string, options: any, callback: any) => {
  ...
  // Explicitly resolve A (IPv4) records first
...
async function startServer() {
  ...
  app.post("/api/smtp/verify", async (req, res) => {
    ...
    debugLogs.push(`[CONFIG] Strict socket timeouts: Connection=5000ms, Greeting=5000ms, Socket=5000ms`);
    ...
    try {
      ...
      // Wrap verification in a strict 5200ms Promise.race to guarantee fast failure
