...
async function startServer() {
  ...
  app.post("/api/smtp/verify", async (req, res) => {
    ...
    try {
      ...
      return res.json({
        success: true,
        message: `SMTP Connection & Authentication SUCCESSFUL! Authenticated with ${cleanHost}:${portNum} as ${cleanUser}.`
      });
    } catch (err: any) {
      let friendlyError = err.message || String(err);
      const lowerErr = friendlyError.toLowerCase();

      if (friendlyError.includes("535") || lowerErr.includes("eauth") || lowerErr.includes("authentication") || lowerErr.includes("credentials")) {
        friendlyError = `Authentication Failed (535) for ${cleanUser} on ${cleanHost}:${portNum}.\n\n` +
          `💡 Microsoft 365 / Outlook Checklist:\n` +
          `1. Ensure 'Authenticated SMTP' is enabled in Microsoft 365 Admin Center for this mailbox (Users -> Active Users -> [User] -> Mail tab -> Manage email apps -> Check 'Authenticated SMTP').\n` +
          `2. If Multi-Factor Authentication (2FA) is enabled on your Microsoft account, generate an App Password at https://mysignins.microsoft.com/ and use it here instead of your normal password.`;
      } else if (lowerErr.includes("etimedout") || lowerErr.includes("econnrefused") || lowerErr.includes("esocket") || lowerErr.includes("timeout")) {
        friendlyError = `Connection Timeout to ${cleanHost}:${portNum} (failed fast within 5s).\n\n` +
          `💡 Office 365 Connection Tips:\n` +
          `1. Verify Host is 'outlook.office365.com' or 'smtp.office365.com'.\n` +
          `2. Ensure Port is set to 587 with STARTTLS enabled (or Port 465 with Direct SSL).\n` +
          `3. Note: Cloud sandboxes can restrict outbound SMTP ports (25/587). The app will connect seamlessly when deployed to your cPanel / Passenger production environment.`;
      } else if (lowerErr.includes("ssl") || lowerErr.includes("tls") || lowerErr.includes("wrong version")) {
        friendlyError = `SSL/TLS Handshake Error on ${cleanHost}:${portNum}.\n\n` +
          `💡 For Port 587, uncheck 'Direct SSL/TLS' and check 'STARTTLS'. For Port 465, check 'Direct SSL/TLS'.`;
      }
